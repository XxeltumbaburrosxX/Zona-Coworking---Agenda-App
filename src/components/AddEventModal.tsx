...
import { X, Calendar as CalendarIcon, Clock, Users, User, Layout, Tags, Trash2 } from 'lucide-react';
...
export function AddEventModal({ onClose, selectedDateStr, editingEvent }: Props) {
  ...
  return (
    <>
      <div className={`fixed inset-x-0 bottom-0 z-50 flex flex-col items-center justify-end sm:justify-center p-0 sm:p-4 bg-slate-900/40 backdrop-blur-sm transition-opacity duration-300 ${isClosing ? 'opacity-0' : 'opacity-100'}`}>
      <div className={`bg-white rounded-t-3xl sm:rounded-3xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] sm:max-h-[85vh] transition-transform duration-300 ${isClosing ? 'translate-y-full sm:translate-y-8 sm:scale-95' : 'translate-y-0 sm:scale-100'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 custom-scrollbar pb-8">
                ...
                <div className="mt-4 space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-4 gap-2 mt-2">
                        <label className="flex items-center justify-center gap-1.5 text-xs text-slate-700 font-medium cursor-pointer p-2 rounded-xl bg-slate-50 border border-slate-200 h-10 min-h-[48px] sm:min-h-0">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2}
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-brand-blue/20 focus:border-brand-blue transition-all resize-none min-h-[48px]" 
        ...
        <div className="p-4 sm:p-6 border-t border-slate-100 bg-slate-50 flex flex-col-reverse sm:flex-row sm:items-center justify-between gap-3 shrink-0 pb-[max(1rem,env(safe-area-inset-bottom))] sticky bottom-0 z-10 w-full">
          {editingEvent ? (
            <button type="button" onClick={handleDelete} disabled={deleting} className="w-full sm:w-auto justify-center px-4 py-3 h-12 min-h-[48px] rounded-xl text-sm font-bold text-red-500 hover:text-red-700 hover:bg-red-50 flex items-center gap-2 transition-colors disabled:opacity-50 border border-red-100 sm:border-transparent bg-white sm:bg-transparent shadow-sm sm:shadow-none">
              <Trash2 size={18} /> Eliminar
            </button>
          ) : <div className="hidden sm:block"></div>}
          
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <button type="button" onClick={handleClose} className="w-full sm:w-auto px-6 py-3 h-12 min-h-[48px] rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-200 transition-colors bg-white sm:bg-slate-100 border border-slate-200 sm:border-transparent shadow-sm sm:shadow-none">
              Cancelar
            </button>
            <button type="submit" form="reservation-form" disabled={loading} className="w-full flex-1 sm:flex-none sm:w-auto justify-center px-6 py-3 h-12 min-h-[48px] rounded-xl text-sm font-bold text-white bg-brand-orange hover:bg-[#E68505] shadow-md shadow-orange-500/20 transition-all disabled:opacity-70 disabled:pointer-events-none flex items-center gap-2">
